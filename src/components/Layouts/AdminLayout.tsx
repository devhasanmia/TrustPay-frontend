...
import { useGetAdminQuery } from "../../redux/api/features/user/userApi";

const AdminLayout = () => {
  const [balanceVisible, setBalanceVisible] = useState(false);
  const dispatch = useAppDispatch();
  const {data: getAdmin} = useGetAdminQuery("");
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <section className="mb-8">
          <h2 className="text-2xl font-semibold text-gray-800 mb-6">Welcome, {getAdmin?.data?.name} {getAdmin?.data?.accountType} </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow">
                ...
                <p className="text-4xl font-bold mt-4">
                  {balanceVisible ? `$${getAdmin?.data?.income.toLocaleString()}` : "****"}
                </p>
                <button
              ...
              <div className="bg-gradient-to-r from-teal-500 to-green-500 text-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow">
                ...
                <p className="text-4xl font-bold mt-4">
                  {balanceVisible ? `$${getAdmin?.data?.balance.toLocaleString()}` : "****"}
                </p>
                <button
                  onClick={toggleBalanceVisibility}
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow">
              ...
              <div className="grid grid-cols-2 gap-4">
                <QuickActionBox
                  ...
                  text="Agents Request"
                  to="/admin/agents-approval-request"
                />
                <QuickActionBox
                  icon={<FaUsers className="text-4xl text-blue-600" />}
                  text="Agent List"
