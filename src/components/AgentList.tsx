...
import { useAgentsApprovalMutation, useGetAgentsQuery } from "../redux/api/features/user/userApi";

const AgentList = () => {
  const { data: getAgents, isLoading,  } = useGetAgentsQuery("");
 ...
 const handleStatusChange = async (key: any, newStatus: string) => {
  ...
  const tableData = getAgents?.data?.map((item: any) => ({
    key: item._id,
    name: item.name,
    email: item.email,
